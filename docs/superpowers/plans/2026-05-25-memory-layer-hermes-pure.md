...
Lumen 启动时，`lib/memory/loader.py` 扫描上述目录，按 `config.yaml` 中 `memory.provider` 的值加载对应插件。`plugin.yaml` 示例：
...
2. **外部 Provider 最多一个**：`MemoryManager.add_provider()` 检查 `provider.name != "builtin"`，拒绝第二个外部 provider，避免 schema 膨胀和冲突。
