...
> 日期：2026-05-25  
> 状态：**已实现**（2026-06-12 同步代码）  
> 范围：用 Hermes 风格的文件优先记忆系统，替换 Lumen 当前基于 `GrowthEvent` 的记忆运行时。  
> 注意：下文保留原始设计记录；与最终实现不一致处已在「实现更新（2026-06-12）」章节标注。
...
- 内置文件记忆永远可用；
- 外部记忆是可选能力，通过**一个或多个**当前激活的 `MemoryProvider` 接入；
...
1. **多外部 provider 共存**：`MemoryManager` 不再限制外部 provider 数量，使用配置实例名 `name` 作为内部 key，支持同类型多实例（如 `honcho-prod` + `honcho-dev`）。`prefetch_all()` / `sync_all()` / `on_memory_write()` 均 fan-out 到所有已启用 provider。
2. **内置插件目录**：除 `~/.lumen/plugins/memory/<name>/` 外，增加 `lib/memory/builtins/<name>/` 作为内置 provider 目录；同名时用户插件覆盖内置插件。Honcho 与 Akasha 均以内置插件形式维护。
3. **配置格式**：由单个 `memory.provider` 改为 `~/.lumen/config.json["memory_providers"]` 列表，每个条目含 `{name, provider_type, enabled, config}`。旧 `honcho_enabled=true` / `HONCHO_API_KEY` 在首次启动时自动迁移。
4. **Akasha 内置插件**：新增 `lib/memory/builtins/akasha/`，提供图记忆引擎的 L2 召回和轮次持久化，默认 sidecar DB 为 `~/.lumen/memory/{user_id}/akasha.db`。
5. **Provider 接口微调**：接口方法并非全部 `async`；`name` 为 property，核心钩子如 `is_available()`、`prefetch()`、`sync_turn()`、`on_memory_write()` 为 `async`，工具 schema / handle 等方法保持同步（与工具系统对齐）。
6. **新 REST 端点**：新增 `/api/memory/providers*`（CRUD / test / reload）和 `/api/akasha/*`（检索概览），详见 `AGENTS.md` API 列表。
7. **写入镜像**：`memory_save` / `update_profile` 写入 `memory.md` 后调用 `MemoryManager.on_memory_write()`，将事件镜像到所有外部 provider。
8. **Embedding 客户端**：新增 `lib/llm/embeddings.py`，Akasha 等插件复用 Lumen 的 embedding 配置。
...
| `loader.py` | 从 `lib/memory/builtins/<name>/` 与 `~/.lumen/plugins/memory/<name>/` 发现 provider 插件；用户插件覆盖同名内置插件。 |
...
Provider 失败必须隔离。坏掉的 provider 只能记录 warning 并降级到内置文件记忆，不能打断聊天；其他 provider 不受影响。

可同时激活多个外部 provider。内置文件记忆（`"builtin"`）始终激活。
...
Lumen 是常驻 asyncio 进程：`MessageBus` → `AgentRunner` 消费循环 → 自研 Agent → `EventBus` → 各 channel（web SSE / telegram / cli）+ presence 主动行为。FastAPI 只是 web channel 的传输层。由此确定记忆作用域：
