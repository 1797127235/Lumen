...
支持 LongMemEval 格式（与 akashic-agent/eval/longmemeval 兼容）：
  question_id, question_type, question, answer, question_date,
  haystack_session_ids, haystack_dates, haystack_sessions, answer_session_ids

haystack_sessions 是 session 列表；每个 session 是 turn 列表；
每个 turn 是 {"role": "user"/"assistant", "content": "..."}。
"""
...
def load_dataset(path: Path | str) -> list[LMEInstance]:
    ...
    for item in raw:
        haystack_sessions = [_parse_turns(s) for s in (item.get("haystack_sessions") or [])]
        instances.append(
            LMEInstance(
