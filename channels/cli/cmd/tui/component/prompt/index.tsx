...
export function Prompt(props: PromptProps) {
  ...
  // Software cursor blink — disables terminal-native blink (whose rate we can't control)
  // and implements a slower, calmer blink at BLINK_MS per half-cycle.
  const BLINK_MS = 600
  ...
  createEffect(() => {
    ...
    blinkTimer = setInterval(() => {
      if (!input || input.isDestroyed) { clearInterval(blinkTimer); blinkTimer = undefined; return }
      visible = !visible
      input.cursorColor = visible ? theme.text : theme.backgroundElement
    }, BLINK_MS)
  })
  onCleanup(() => { if (blinkTimer) { clearInterval(blinkTimer); blinkTimer = undefined } })
  ...
  const usage = createMemo(() => {
    ...
    const last = msg.findLast((item): item is AssistantMessage => item.role === "assistant" && (item.tokens?.output ?? 0) > 0)
    if (!last) return

    const tokens =
      (last.tokens?.input ?? 0) + (last.tokens?.output ?? 0) + (last.tokens?.reasoning ?? 0) + (last.tokens?.cache?.read ?? 0) + (last.tokens?.cache?.write ?? 0)
    if (tokens <= 0) return
  ...
  async function submitInner() {
    ...
    if (store.mode === "shell") {
      ...
      const [rawCommand, ...argParts] = firstLine.split(" ")
      const cmdName = rawCommand.slice(1) // 去掉 /
      const args =
        argParts.join(" ") + (firstLineEnd === -1 ? "" : "\n" + inputText.slice(firstLineEnd + 1))

      // 检查是否是已注册的命令（用于过滤掉非命令的 /xxx 输入）
      const isKnown = sync.data.command.some((x) => x.name === cmdName)
      if (!isKnown) {
        // 不是已知命令，当普通消息发送
        sdk.client.session
          .prompt({
        ...
        try {
          const result = await LumenApi.executeCommand(cmdName, args.trim(), sessionID)

          if (!result.ok) {
            toast.show({ message: result.error || "命令执行失败", variant: "error", duration: 3000 })
            return false
          }

          switch (result.action) {
            case "switch":
              if (result.session_id) route.navigate({ type: "session", sessionID: result.session_id })
              break
            case "delete":
          ...
          toast.show({ message: err.message || "命令执行失败", variant: "error", duration: 3000 })
          return false
        }
      }
    } else {
