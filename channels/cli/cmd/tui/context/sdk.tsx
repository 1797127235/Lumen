    ...
    const client = {
      session: {
        ...
        async delete(opts: { sessionID: string }) {
          const convID = sessionIDMap.get(opts.sessionID) ?? opts.sessionID
          try {
      ...
      auth: {
        async set(opts: { providerID: string; auth: { type: string; key: string; metadata?: Record<string, string> } }) {
          try {
            const res = await fetch(`${LumenApi.BASE_URL}/config`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                providers: {
                  [opts.providerID]: { api_key: opts.auth.key },
                },
              }),
            })
            if (!res.ok) return { data: undefined, error: new Error(`保存失败: ${res.statusText}`) }
          } catch (e) {
