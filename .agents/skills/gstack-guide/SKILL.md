...
description: |
  ...
  /ship /qa /review /browse /cso /autoplan, or is trying to set up Garry Tan's Codex
...
Garry Tan（Y Combinator CEO）开源的 AI 工程工作流框架，将 Codex 变成一支虚拟工程团队。
...
- 把 Codex 变成虚拟团队：CEO、设计师、工程经理、安全官、QA、发布工程师
- 92K+ GitHub stars，MIT 协议，免费开源
- 支持 Codex、Codex、Cursor、OpenCode 等 8+ 个 AI 代理平台
...
然后让 Codex 在 `AGENTS.md` 中添加 gstack 章节，列出所有可用技能。
...
git add .Codex/ AGENTS.md && git commit -m "require gstack for AI-assisted work"
```

团队成员安装 gstack 后，每次启动 Codex 会自动同步最新版本。
...
**Codex 说看不到技能？**
- 确保 `AGENTS.md` 中有 gstack 章节
- 检查 `~/.Codex/skills/gstack/` 是否存在
