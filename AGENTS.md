...
5. **Honcho 同步**：`HonchoProvider`（`lib/memory/honcho_provider.py`）在 `startup.py` 中自动注册，提供 L0 画像摘要、L2 语义召回、轮次同步、记忆镜像
6. **定期整理**：`MemoryHousekeeper`（`lib/memory/housekeeping.py`）24 小时循环清理：`[intent]` 30 天后标 stale、`[transient]` 7 天后删除、`[fact]`/`[preference]` 永不过期
...
│   ├── startup.py              # 启动初始化（建表、Honcho 注册、Channel 启动、关闭清理）
...
│   │   ├── honcho_provider.py  # HonchoProvider（Honcho AI 外部记忆，L0+L2+轮次同步）
...
│   ├── partner/                # 伙伴系统（情绪、主动对话、RSS 推送）
│   │   ├── models.py           # LumenState + LumenPresence + LumenThought ORM
│   │   ├── mood_inference.py   # 情绪推断逻辑
│   │   ├── presence.py         # 在线状态与主动触发管理
│   │   ├── proactive_scheduler.py # ProactiveScheduler（频率门控 + 每日限额的主动推送）
│   │   └── rss_reader.py       # RSS 条目读取 + LLM 筛选 + 朋友式推送消息生成
│   ├── config/                 # 配置模块
│   │   └── service.py          # 配置业务逻辑
...
| `HONCHO_API_KEY` | Honcho AI 外部记忆服务密钥 |
...
| `proactive_enabled` | 主动推送开关（`true` 时启用 ProactiveScheduler）|
| `rss_enabled` / `rss_fetch_interval` | RSS 功能开关与抓取间隔 |
...
- `HonchoProvider` 提供语义召回（通过 `peer.chat()` 查询用户相关记忆）
...
- `HonchoProvider.sync_turn()` 将每轮对话写入 Honcho session，`on_memory_write()` 将主动写入的记忆镜像到 Honcho
