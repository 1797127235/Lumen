...
│           ├── config.ts
│       └── partner.ts
├── channels/                    # CLI TUI（独立 TypeScript/Bun 项目）
│   └── cli/cmd/tui/
...
│       │   ├── sync.tsx         # SSE 事件 → OpenCode store 映射层；启动时加载斜杠命令
...
│       │   ├── prompt/          # 输入框（含斜杠命令解析与分发）
│       │   └── dialog-session-list.tsx  # 会话列表对话框（模糊搜索/删除/重命名/置顶）
│       └── stubs/               # OpenCode SDK 接口桩（适配 Lumen 后端）
