...
4. **外部召回 / Provider 插件化**：如需语义检索，把 `MemoryProvider` 插件放到 `lib/memory/builtins/<name>/`（内置）或 `~/.lumen/plugins/memory/<name>/`（用户）；启动时按 `~/.lumen/config.json["memory_providers"]` 动态加载。**Lumen 当前最多同时启用一个外部 memory provider**（避免同名工具冲突），builtin 文件记忆始终存在。`MemoryManager` 在 L1 prefetch 时调用外部 provider
...
**多实例并存**：`name` 是配置实例名（如 `honcho-prod`、`honcho-dev`），`provider_type` 是插件类型。多个配置可以共用同一个 `provider_type`，只要 `name` 不同就不会互相覆盖。`MemoryManager` 用 `name` 作为内部 key，用 `provider.display_name` 标注 L0/L1 结果。
...
│   ├── provider.py             # ChannelProvider 抽象基类
│   ├── manager.py              # ChannelManager：发现/加载/生命周期
│   ├── models.py               # ChannelConfig 配置模型
│   ├── config_store.py         # channels 配置持久化
│   ├── builtins/               # 内置 Channel Provider
│   │   ├── web/                # Web 渠道
│   │   │   ├── __init__.py     # WebChannelProvider
│   │   │   ├── web.py          # WebChannel：SSE 流式对话
│   │   │   └── formatters.py   # SSE 事件格式化
│   │   └── telegram/           # Telegram 渠道
│   │       ├── __init__.py     # TelegramChannelProvider
...
| `GET` | `/api/channels` | 列出已发现/已配置的 channel |
| `POST` | `/api/channels` | 添加 channel 配置 |
| `PUT` | `/api/channels/{name}` | 更新 channel 配置 |
| `DELETE` | `/api/channels/{name}` | 删除 channel 配置 |
| `POST` | `/api/channels/{name}/test` | 测试 channel 配置 |
...
| `memory_review_interval` | 后台记忆审查间隔（消息数）。1=每轮都审，10=每10条消息审一次 |
...
- **BaseChannel**（`channels/base.py`）：抽象基类，`start()`/`stop()`/`send_message()`，新增 `name` / `instance_name` / `capabilities()`
- **ChannelProvider**（`channels/provider.py`）：Channel 插件工厂，`build(config, bus, event_bus) -> BaseChannel`
- **ChannelManager**（`channels/manager.py`）：扫描 `channels/builtins/` 和 `~/.lumen/plugins/channels/`，按 `~/.lumen/config.json["channels"]` 启动 channel
- **WebChannel**（`channels/builtins/web/web.py`）：SSE 流式，前端 / Tauri 接入
- **TelegramChannel**（`channels/builtins/telegram/channel.py`）：Polling 模式，缓冲流式增量，整条发送
...
**添加新 Channel**：
1. 在 `channels/builtins/<name>/` 下创建 `__init__.py` 实现 `ChannelProvider`
2. 实现对应的 `BaseChannel` 子类
3. 添加 `plugin.yaml`（name/version/class）
4. 重启后端，在 `~/.lumen/config.json["channels"]` 中增加配置或调用 `POST /api/channels`

用户插件可放在 `~/.lumen/plugins/channels/<name>/`，同名时覆盖内置插件。

> 注：`channels/web/` 和 `channels/telegram/` 仍保留为兼容 shim，实际实现已移至 `channels/builtins/`。新增 channel 请直接放在 `channels/builtins/<name>/`。
...
- `USER.md`（缺失时降级到 `MEMORY.md`），由 `MemoryManager.build_system_prompt()` 经 `BuiltinMemoryProvider` 读取
