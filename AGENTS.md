...
**技术栈**: FastAPI + SQLAlchemy + SQLite | React 19 + Vite + Tailwind CSS 4 | Tauri v2 (Rust)
...
4. **外部召回 / Provider 插件化**：如需语义检索，把 `MemoryProvider` 插件放到 `lib/memory/builtins/<name>/`（内置）或 `~/.lumen/plugins/memory/<name>/`（用户）；启动时按 `~/.lumen/config.json["memory_providers"]` 动态加载。**设计约束：最多 1 个外部 provider 并存**（软限制）——配置仍是数组（向前兼容），但启动时只启用第一个 `enabled=True` 的外部配置，后续的忽略并记 warning；API 层（create/update/reload）也会拒绝导致第二个并存的操作。builtin（文件记忆 `MEMORY.md`）始终保留，外部 provider 是叠加
5. **定期整理**：`MemoryHousekeeper`（`lib/memory/housekeeping.py`）24 小时循环清理：`[intent]` 30 天后标 stale、`[transient]` 7 天后删除、`[fact]`/`[preference]` 永不过期

### 人格系统（PERSONA.md）

人格（性格、语气、说话方式）是 Lumen 唯一的真相源,**不硬编码在代码里**。和 MEMORY.md / USER.md / PARTNER.md 一样,是 `~/.lumen/memory/{user_id}/PERSONA.md` 这个可编辑 Markdown 文件。

**设计原则**:人格来源唯一、不冲突。系统里只有一处定义 Lumen 的性格——要么是 PERSONA.md(用户写了),要么是默认人格常量(`system_prompt_builder.py` 的 `_DEFAULT_PERSONA`,PERSONA.md 为空时回退),**两者二选一,绝不叠加**。

**注入位置**:`<persona>` 块注入在 system prompt 的 **context_suffix 层**(和 `<partner-rules>`、用户画像同级),不是稳定前缀层。这意味着:
- 同一对话内 system prompt 冻结,改了 PERSONA.md 要**新开对话**才生效
- `_build_identity_block()` 里不再包含性格段落,只保留身份声明(身份 + 一句"人格在下方 `<persona>` 块给出")和一句指引"用户自定义人格存在时以它为准,否则用默认"

**修改人格**:
- 直接编辑 `~/.lumen/memory/{user_id}/PERSONA.md`,或
- `PUT /api/memory/persona` body `{ "content": "..." }`(写入后自动 `invalidate_system_prompt_cache`,新对话生效)
- 读取用 `GET /api/memory/persona`,或 `AsyncMarkdownStore().read_persona(user_id)`

**字符上限**:1500(`_MD_CHAR_LIMITS["persona"]`),超出自动截断;写入时经过 `_scan_memory_content` 安全扫描(prompt injection / 数据外泄 / 隐形 Unicode),镜像 PARTNER.md 的保护机制。

**默认人格**:就是 2026-06-17 重构前硬编码在 `_build_identity_block()` 里的那段("先接住再展开""会轻轻吐槽""情绪要看得见")。搬成 `_DEFAULT_PERSONA` 常量后,用户什么都不写时行为和重构前完全一致——这是人格系统的安全网。
...
**设计约束：最多 1 个外部 provider 并存（软限制）**：`name` 是配置实例名，`provider_type` 是插件类型。虽然配置文件仍为数组（向前兼容历史 schema），运行时只允许 1 个外部 provider 启用——builtin 始终保留，外部是叠加而非替换。约束在 3 个入口强制：
- **启动时**（`core/startup.py`）：遍历配置，遇到第一个 `enabled=True` 的外部配置即占名额，后续 enabled 配置忽略并 warning。即使第一个进了 pending 队列（不可用），后续的也不会被启用——否则 reconciler 重连成功会出现 2 个并存。
- **create/update API**（`server/routes/memory_providers.py`）：若操作会导致第 2 个 enabled 配置出现，返回 409 Conflict。
- **reload API**：若运行时已有其他外部 provider 实例，拒绝 reload。

`MemoryManager` 用 `instance_name` 作为内部 key，`provider.display_name` 标注 L0/L2 结果。
...
│   │   ├── markdown.py         # AsyncMarkdownStore：MEMORY.md / USER.md / PARTNER.md / PERSONA.md 原子读写
...
│   ├── scheduler/              # 主动行动 — 时间触发（APScheduler 调度）
│   │   ├── __init__.py         # get_scheduler_engine(bus) 全局单例
│   │   ├── store.py            # TaskSpec 持久化（含 notify_* 目标字段）
│   │   ├── engine.py           # SchedulerEngine：APScheduler 封装
│   │   └── executor.py         # run_task：到点调工具 + 可选 notify 注入对话
│   ├── triggers/               # 主动行动 — 事件触发（MCP notification / 短轮询）
│   │   ├── __init__.py         # get_manager(bus) 全局单例
│   │   ├── store.py            # SubscriptionSpec 持久化（含 target_* 推送目标）
│   │   └── listener.py         # TriggerManager：监听 + 匹配订阅 + 注入对话
│   ├── config/                 # 配置模块
│   │   └── service.py          # 配置业务逻辑
...
│       ├── scheduler.py        # schedule / list_schedules / cancel_schedule（时间触发）
│       ├── triggers.py         # subscribe_events / list / cancel（事件触发）
│       └── mcp/                # MCP 工具桥接
│           ├── client_manager.py
│           ├── config_store.py
│           ├── models.py
...
├── mcp_servers/                # 内置 MCP 服务器目录（当前为空）
├── observatory/                # 观测站 — 单文件 HTML（Chart.js + Tailwind CDN，零构建）
├── server/                     # API 路由层
│   └── routes/
...
| `GET` | `/api/memory/persona` | 读取人格设定（PERSONA.md） |
| `PUT` | `/api/memory/persona` | 保存人格设定（写入后新对话生效） |
...
| `GET` | `/api/health` | 健康检查（真实 DB ping + 版本号） |
| `GET` | `/api/metrics/summary?range=` | KPI 摘要（token / cost / 对话数 / 平均延迟 / 错误率），range: 1h/24h/7d/30d |
| `GET` | `/api/metrics/usage?range=&bucket=` | token 用量时序（input/output/cache_read/cache_write），bucket: hour/day |
| `GET` | `/api/metrics/latency?range=&bucket=` | 延迟时序（turn/llm/tool/http 各一条线） |
| `GET` | `/api/metrics/tools?range=` | 工具调用排行（按 tool_name 聚合调用数/平均耗时/错误数） |
| `GET` | `/api/metrics/events?name=&page=&page_size=` | 原始事件流分页（page_size 上限 200） |
...
- **AgentRunner**（`lib/chat/agent_runner.py`）：后台消费 MessageBus，驱动自研 Agent Loop
...
> 注意：`lumen-feed` 是独立的常驻 MCP 应用，源码在 `apps/feed/`。需先启动 `python apps/feed/server.py`，再在 Lumen 中配置 SSE 接入。
...
长期记忆本身不再走 SQLite，而是文件优先（`MEMORY.md`）。SQLite 现承载对话/消息、外部文档索引、伙伴系统状态、metric_events 指标表等。外部语义召回由 `MemoryProvider` 插件提供。

### Metrics / 可观测性体系

统一时间序列表 `metric_events` + 进程单例 `MetricsRecorder`。所有埋点走同一个 `record(name, value, labels)` 接口，仪表盘按 name + 时间窗聚合。

**为什么不每类指标建独立表**：避免 agent_traces 那种 schema 错位再次发生；新增指标只需加一个 `record()` 调用，无需改 schema。

**`lib/metrics/` 包结构**：

| 文件 | 职责 |
|------|------|
| `models.py` | `MetricEvent(Base)`：统一时间序列表（name / value / labels_json / created_at） |
| `recorder.py` | `MetricsRecorder` 进程单例 + `record()` 便捷函数（fire-and-forget） |

**`metric_events` schema**：
- `name`（indexed）：指标名，如 `llm.tokens.input` / `turn.duration_ms` / `tool.calls`
- `value`（Float）：计数=1.0 / 耗时 ms / token 数
- `labels_json`（TEXT）：维度 JSON，如 `{"model":"...","tool":"...","channel":"...","status":"ok|error"}`
- `conversation_id` / `user_id`：可选关联（主动消息/后台任务可能为空）
- `created_at`（indexed, tz-aware）：聚合时间键

**埋点位置表**（7 处 chokepoint，覆盖所有关键路径）：

| 指标 | 文件 | 函数 | labels |
|------|------|------|--------|
| `llm.tokens.*` / `llm.call_duration_ms` / `llm.calls` / `llm.context_overflow` | `lib/llm/client.py` | `LLMClient.chat` | model, status |
| `turn.duration_ms` / `turn.completed` | `lib/chat/agent_runner.py` | `_process_message` | channel, outcome |
| `react.iteration` / `react.max_iterations_reached` / `react.context_overflow` | `core/agent.py` | `_run_react_loop` / `run_agent` | iteration_no |
| `tool.duration_ms` / `tool.calls` | `lib/tools/_registry.py` | `ToolRegistry.execute` | tool_name, status |
| `http.request_duration_ms` | `shared/logging.py` | `RequestLoggingMiddleware` | method, path, status |
| `proactive.deliveries` | `lib/proactive/delivery.py` | `deliver` | channel, outcome, source |
| `memory.writes` | `lib/memory/manager.py` | `on_memory_write` | target, action |

**关键设计约束**：
1. **绝不阻塞/破坏业务** —— `MetricsRecorder` 全程 try/except 兜底，失败只 warning。即使 DB 未就绪也不阻断启动。
2. **独立短事务** —— 单例自己开短事务插入，不依赖调用方的 db session（埋点遍布中间件/后台任务，很多地方没有 session 句柄）。
3. **fire-and-forget** —— 高频路径（LLM 调用）用 `asyncio.create_task` 异步插入，调用方不等待 DB。`_pending` set 保留 task 强引用避免 GC。
4. **`shared/logging.py` 延迟导入 metrics** —— `shared/` 是底层模块不能依赖 `lib/`，`record` 在 dispatch 方法内部 lazy import 打破循环。
5. **统一 `metric_events` 表** —— 新增埋点不需改 schema，只加 `record()` 调用 + 在此表登记。

**新增埋点步骤**：
1. 在 chokepoint 调用 `from lib.metrics import record`
2. `await record("your.metric.name", value, labels={...})`
3. 在上面的埋点位置表登记
4. 重启后端生效

**启动注册**：`core/startup.py` 在 `init_db()` 之后、业务组件启动之前 `set_recorder(MetricsRecorder())`。

**观测站**：`observatory/index.html`（单文件 HTML，纯 Chart.js + Tailwind CDN，零构建），主后端 `main.py` 挂载在 `/observatory` 路由。与主 React 前端完全解耦——主前端不维护也能用，主力 Telegram 渠道随时打开浏览器访问 `http://localhost:8000/observatory` 即可查看。每 30s 自动刷新。

### 主动行动能力（时间触发 + 事件触发）

Lumen 默认是被动应答（用户说话才有反应）。主动行动能力让 AI 伙伴能**自己到点做事**或**有事件时主动找你**，是"伙伴感"的核心。
...
```
  ...
  │ 事件(triggers) │    │ notify → 注入对话 → agent 主动找你 │
  └──────────────┘    └──────────────────────────┘
```

两个模块是一对对称的家族，**不合并代码**（触发机制不同：APScheduler vs asyncio 短轮询），但在**动作层统一**：`notify` 动作共享同一条「注入 `InboundMessage` → `AgentRunner` 跑 agent → `OutboundMessage` → Telegram 推送」路径。

| 模块 | 触发源 | 动作 | 工具 |
|------|--------|------|------|
| `lib/scheduler/` | 时间（cron / interval / date） | silent（默认）或 notify | `schedule` / `list_schedules` / `cancel_schedule` |
| `lib/triggers/` | 事件（MCP server notification / 短轮询） | notify | `subscribe_events` / `list_subscriptions` / `cancel_subscription` |

**关键约束：**

- **启动顺序**：`core/startup.py` 在 MCP `connect_all()` **之前**初始化 `TriggerManager`，确保 MCP server 连上后，通知 handler 能解析到它。两个引擎都包在 `contextlib.suppress(Exception)` 里，失败不阻断启动。
- **命名避坑**：`lib.scheduler.get_scheduler_engine()`（曾叫 `get_engine`，与 `core.db.get_engine` 同名）。保留 `get_engine` 别名仅为向后兼容，新代码请用 `get_scheduler_engine`。
- **MCP notification 转发**：`McpClientManager._make_message_handler(name)` 在 `ClientSession` 构造时注入，过滤 `ServerNotification` 后延迟转发给 `get_manager().handle_notification(name, notification)`。`TriggerManager` 未初始化时静默跳过。
- **目标渠道从对话 ctx 自动捕获**：`schedule(notify=true)` 和 `subscribe_events` 都从当前对话的 `ctx.source_platform` / `ctx.chat_id` / `ctx.user_id` 自动确定推送目标，无需用户手填。
- **Telegram 是当前唯一真正主动触达的渠道**：Bot API 主动推消息，无需用户在线。Web 渠道的主动消息目前是**静默丢弃**（`WebChannel._streams` 是 SSE 瞬态，只在用户正开着那个会话时才送达；reply 会持久化进历史，事后打开可见）。真正的 Web 推送需要持久化收件箱/通知表，是独立的后续工程。
- **持久化恢复**：任务/订阅分别落 `~/.lumen/scheduler/jobs.json` 和 `~/.lumen/triggers/subscriptions.json`，重启后自动恢复并重新起调度/轮询。
- **结果可追溯**：scheduler 每次执行追加记录到 `~/.lumen/scheduler/results/<task_id>.jsonl`（含 ok/error/时间戳）。
...
- **单用户模式**：身份固定为 `"me"`（`lib/identity.py` 的 `SINGLE_USER_ID`），多用户需改造用户管理
...
- **主动行动能力只在 Telegram 真正触达**：`schedule(notify=true)` 和 `subscribe_events` 注入的 `InboundMessage` 经 agent 跑出回复后，只有 TelegramChannel 能主动推送。Web 渠道的 `WebChannel._streams` 是 SSE 瞬态——用户没正开着那个会话页面，主动消息实时送达会被静默丢弃（但 reply 仍持久化进历史，事后打开会话可见）。要支持 Web 实时主动推送，需给 WebChannel 加持久化收件箱/通知表，是独立的后续工程
- **主动引擎必须按顺序启动**：`TriggerManager` 必须在 MCP `connect_all()` **之前**初始化，否则 MCP server 连上时通知 handler 解析不到 manager（虽然 handler 延迟解析 + try/except 兜底不会崩，但通知会被丢弃）。两个引擎的 `start()`/`stop()` 在 `core/startup.py` 的 `lifespan` 中成对出现
