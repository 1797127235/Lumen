...
class AkashaMemoryEngine:
    ...
    async def query(
        ...
        if request.intent == "timeline":
            return MemoryQueryResult(trace={"engine": self.DESCRIPTOR.name, "intent": "timeline_unsupported"})
        ...
        text_block = (
            self._format_context_block(dense_cards, ripple_cards, now_ts=now_ts) if request.intent == "context" else ""
        )
    ...
    def undo_by_message_sources(
        ...
        *,
        ...
        if not dry_run and affected_keys:
            ...
            turns = [parsed for key in keys if (parsed := _parse_turn_key(key)) is not None]
    ...
    def _retrieve(
        ...
        *,
        ...
        source_db = sqlite3.connect(str(self._session_db_path)) if self._session_db_path.exists() else None
    ...
    def _load_graph_cache(self) -> None:
        ...
        message_embeddings = dict(self._store.list_cached_embeddings(model=self._config.memory.embedding.model))
    ...
    def _graph_snapshot(self) -> AkashaActivationSnapshot:
        ...
        with self._graph_lock:
            ...
            return AkashaActivationSnapshot(
                ...
                fan=dict(self._fan),
                edges_by_src={key: dict(value) for key, value in self._edges_by_src.items()},
    ...
    def _write_query_log(
        ...
        *,
        ...
        store.insert_query_log(
            ...
            query_text=request.text.strip(),
            intent=request.intent,
            ts=datetime.fromtimestamp(_query_timestamp_unix(request) or 0.0, UTC).isoformat(),
            seed_count=result.trace.seed_count,
            pool_count=result.trace.pool_count,
    ...
    def _format_context_block(
        ...
        *,
        ...
        if dense_cards or ripple_cards:
            date_label = datetime.fromtimestamp(now_ts, UTC).astimezone().strftime("%Y-%m-%d")
...
def _load_turn_card(
    ...
    *,
    ...
    if user_row is None and assistant_row is None:
        return None
    source_ids = [str(row["id"]) for row in (user_row, assistant_row) if row is not None and str(row["id"]).strip()]
    ...
    happened_at = str(user_row["ts"] or "") if user_row is not None else str(assistant_row["ts"] or "")
    return AkashaCard(
        key=key,
...
def _query_log_id(session_key: str, seq: int, intent: str, query_text: str) -> str:
    ...
    digest = hashlib.sha1(f"{intent}\n{query_text}".encode()).hexdigest()[:10]
...
def _format_cards(title: str, cards: list[AkashaCard]) -> str:
    ...
    for card in cards:
        ...
        lines.append(f"- user={user_text} assistant={assistant_text}" f"{time_part} source_ref={card.source_ref}")
    return "\n".join(lines)
