...
def _render_query_detail(raw: dict[str, object]) -> str:
    ...
    lines = [
        ...
        (f"• 实际激活 (Activated): `{raw.get('activated_count')}` 个" f" | 门槛: `{threshold:.3f}`"),
    ]
    lines.extend(
...
def _json_items(value: object) -> list[dict[str, object]]:
    ...
    if not isinstance(loaded, list):
        return []
    return [cast(dict[str, object], item) for item in cast(list[object], loaded) if isinstance(item, dict)]
