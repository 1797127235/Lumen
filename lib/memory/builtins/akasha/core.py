...
def build_idf_table(conn: sqlite3.Connection) -> dict[str, float]:
    ...
    for (content,) in conn.execute("SELECT user_message FROM akasha_turn_content"):
        ...
        for w in jieba.cut_for_search(content or ""):
            cleaned = "".join(ch for ch in w.strip() if ch.isalnum() or "一" <= ch <= "鿿").lower()
    ...
    *,
    ...
    if message_index is not None:
        ...
        scored = [
            ...
            for message_id, score in zip(message_ids, np.dot(matrix, query_norm), strict=False)
        ]
    else:
        scored = []
        for message_id, embedding in message_embeddings.items():
...
def state_array(
    ...
    for index, key in enumerate(keys):
        ...
        values[index] = math.exp(1.4 * node.salience + 1.0 * long_score) * resource / math.sqrt(1.0 + fan.get(key, 0))
    return values
...
def score_candidates(
    ...
    *,
    ...
    for index, key in enumerate(keys):
        ...
        score = (
            (content_base * gain_salience * gain_long * gain_edge) * resource * hop_penalty * user_penalty / fan_penalty
        )
...
def graph_expand_candidates(
    ...
    for seed_key in graph_seed_keys:
        ...
        out_strength = sum(_eff(seed_key, dst_key, edge_weight) for dst_key, edge_weight in raw_neighbors.items())
        if out_strength <= 0:
            continue
