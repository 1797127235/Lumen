...
CREATE TABLE IF NOT EXISTS akasha_turn_content (
    key                TEXT PRIMARY KEY,
...
class AkashaStore:
    ...
    def upsert_turn_content(
        ...
        with self._lock:
            _ = self._db.execute(
                """
                INSERT INTO akasha_turn_content (key, user_message, assistant_preview, updated_at)
                VALUES (?, ?, ?, ?)
                ...
                """,
                (key, user_message, assistant_preview, now),
            )
            self._db.commit()

    # 读取一轮对话的原文内容。
    def get_turn_content(self, key: str) -> tuple[str, str]:
        with self._lock:
            row = self._db.execute(
                "SELECT user_message, assistant_preview FROM akasha_turn_content WHERE key = ?",
                (key,),
            ).fetchone()
        if row is None:
            return "", ""
        return str(row["user_message"] or ""), str(row["assistant_preview"] or "")
    ...
    def list_cached_embeddings(self, *, model: str) -> list[tuple[str, np.ndarray]]:
        ...
        return [(str(row["message_id"]), deserialize_f32(row["embedding"])) for row in rows]
    ...
    def _load_salience_state_locked(self) -> tuple[np.ndarray | None, int]:
        row = self._db.execute("SELECT vector_sum, count FROM akasha_salience_state WHERE key = 'global'").fetchone()
        if row is None:
            return None, 0
    ...
    def load_edges_with_meta(
        ...
        with self._lock:
            rows = self._db.execute("SELECT src_key, dst_key, weight, last_used_ts FROM akasha_edges").fetchall()
        edges = {(str(row["src_key"]), str(row["dst_key"])): float(row["weight"] or 0.0) for row in rows}
        meta = {(str(row["src_key"]), str(row["dst_key"])): float(row["last_used_ts"] or 0.0) for row in rows}
        return edges, meta
    ...
    def list_query_logs(
        ...
        *,
        ...
        offset = (page - 1) * page_size
        with self._lock:
            count_row = self._db.execute(f"SELECT COUNT(1) FROM akasha_query_log {where}", params).fetchone()
            rows = self._db.execute(
                f"""
