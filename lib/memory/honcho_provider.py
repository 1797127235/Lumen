"""Honcho 记忆 Provider — 集成 Honcho AI 的长期记忆服务。
...
class HonchoProvider(MemoryProvider):
    ...
    def _get_client(self) -> Any:
        ...
        if self._client is None:
            ...
            self._workspace_id = os.getenv("HONCHO_WORKSPACE_ID", "lumen")
            self._environment = os.getenv("HONCHO_ENVIRONMENT", "production")
    ...
    async def prefetch(self, query: str, *, session_id: str = "", **kwargs: Any) -> str:
        ...
        使用 peer.chat() 进行语义查询，获取与当前 query 相关的用户记忆。
        """
        user_id = kwargs.get("user_id", "")
    ...
    async def sync_turn(self, user: str, assistant: str, *, session_id: str = "") -> None:
        ...
        将 user_msg 和 assistant_msg 写入对应的 Honcho session。
        由 Lumen 的 MessageBus 在 assistant turn 结束后调用。
        """
        if not session_id:
            return

        # 清洗 session_id 以符合 Honcho 要求
        honcho_session_id = self._sanitize_session_id(session_id)
        ...
        try:
            ...
            # 从 session 的 peers 中找 user peer
            peers = session.peers()
            user_peer = None
            assistant_peer = None
    ...
    async def get_tool_schemas(self) -> list[dict]:
        ...
        记忆查询通过 prefetch() 自动进行，无需 Agent 手动调用。
        """
        return []
    ...
    async def on_memory_write(self, action: str, target: str, content: str, metadata: dict | None = None) -> None:
        """当 builtin 写入 MEMORY.md 时，镜像到 Honcho。

        将用户主动分享的信息（通过 memory_save / tellAI）同步到 Honcho，
        增强 Honcho 的 representation。
        """
        if action != "save" or not content:
            return
        ...
        try:
            ...
            memory_session.add_messages(
                [
                    peer.message(f"[用户主动分享的记忆] {content}"),
                ]
            )
    ...
    async def get_stats(self, user_id: str) -> dict:
        ...
        try:
            ...
            # 获取结论数量
            conclusions = peer.conclusions.list()
            conclusion_count = len(conclusions) if hasattr(conclusions, "__len__") else 0

            # 获取会话数量
            sessions = peer.sessions()
            session_count = len(sessions) if hasattr(sessions, "__len__") else 0
            ...
            logger.debug("Honcho stats", **stats)
            return stats
        except Exception as exc:
            logger.warning("Honcho get_stats 失败", user_id=user_id, error=str(exc))
            return {"user_id": user_id, "available": False, "error": str(exc)}

    async def shutdown(self) -> None:
        """关闭 Provider。Honcho SDK 无需显式关闭。"""
        self._client = None
