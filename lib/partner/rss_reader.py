...
from lib.tools.web_tools import _do_extract
...
class RSSReaderAgent:
    ...
    def __init__(self) -> None:
        self._agent: Agent[RSSReaderDeps, str] | None = None
...
- **不暴露工具状态**：禁止出现"无法提取"、"提取失败"、"页面无法访问"、"Telegram 转发"、"原文获取不到"、"没有详细内容"等描述
- **提炼核心观点**：不说原文写了什么，说**为什么值得看**
- **自然带链接**：链接放在文案末尾，不要突兀
- **有消息就推，没有就不推**，不要出现与推送无关的内容
...
只输出最终推送文案，不要任何解释、不要 JSON、不要代码块、不要思考过程。
...
```"""
    ...
    def _create_model(self) -> OpenAIChatModel:
        """创建模型实例 —— 复用主 Agent 的配置逻辑"""
        return _lumen_agent._create_model()

    def _get_agent(self) -> Agent[RSSReaderDeps, str]:
        """获取或创建 Agent 实例"""
        if self._agent is None:
            model = self._create_model()
            self._agent = Agent(
                model=model,
                deps_type=RSSReaderDeps,
                output_type=str,
            ...
            # 注册 web_extract 工具
            @self._agent.tool
            async def web_extract(ctx: RunContext[RSSReaderDeps], urls: list[str]) -> str:
                """提取网页完整内容"""
                if not urls:
                    return "❌ 请提供 URL"

                results = []
                for url in urls[:3]:  # 最多 3 个
                    try:
                        data = await asyncio.wait_for(_do_extract(url), timeout=25)
                        if "error" in data:
                            results.append(f"❌ {url}: {data['error']}")
                            continue
                        ...
                        results.append(f"❌ {url}: 提取超时")
                    except Exception as e:
                        results.append(f"❌ {url}: {e!s}")
...
如果都不值得推，只回复："（无输出）"。"""
        ...
        try:
            ...
            deps = RSSReaderDeps(user_id=user_id, focus=focus)
