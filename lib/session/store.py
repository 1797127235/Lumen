...
class SessionStore:
    ...
    def _ensure_session_columns(self) -> None:
        ...
        # 移除已退役的 RSS/主动推送相关列
        for col in ("last_user_at", "last_proactive_at"):
            if col in existing:
                with contextlib.suppress(sqlite3.OperationalError):
                    self._conn.execute(f"ALTER TABLE sessions DROP COLUMN {col}")
        if "next_seq" not in existing:
            self._conn.execute("ALTER TABLE sessions ADD COLUMN next_seq INTEGER NOT NULL DEFAULT 0")
    ...
    def _ensure_fts(self) -> None:
        try:
            ...
            if existing:
                try:
                    cfg = dict(self._conn.execute("SELECT * FROM messages_fts_config").fetchall())
    ...
    def session_exists(self, key: str) -> bool:
        with self._lock:
            row = self._conn.execute("SELECT 1 FROM sessions WHERE key = ?", (key,)).fetchone()
    ...
    def get_channel_metadata(self, channel: str) -> list[dict[str, Any]]:
        like_key = f"{channel}:%"
        with self._lock:
            rows = self._conn.execute("SELECT key, metadata FROM sessions WHERE key LIKE ?", (like_key,)).fetchall()
    ...
    def insert_message(
        ...
        *,
        ...
        tool_chain_payload = json.dumps(tool_chain, ensure_ascii=False) if tool_chain is not None else None
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                next_seq = max(int(row["seq"]) for row in seq_rows) + 1 if seq_rows else 0
                cur = self._conn.execute(
                    f"""
    ...
    def search_messages(
        ...
        *,
        ...
        score_expr = " + ".join("(CASE WHEN m.content LIKE ? THEN 1 ELSE 0 END)" for _ in terms)
        if self._has_fts:
