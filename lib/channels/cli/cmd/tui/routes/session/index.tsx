...
function AssistantMessage(props: { message: Message; parts: Part[] }) {
  ...
  // Only show when there's actual non-empty content (not just an empty shell)
  const hasContent = () =>
    props.parts.some((p) => {
  ...
  return (
    <box flexDirection="row" height={dimensions().height} width={dimensions().width}>
      ...
      <box flexDirection="column" flexGrow={1}>
        ...
        {/* Separator */}
        <box height={1} backgroundColor={theme.backgroundPanel}>
          <text fg={theme.borderSubtle} overflow="hidden" wrapMode="none">
            {"─".repeat(dimensions().width)}
          </text>
        </box>

        {/* Footer: directory + status */}
