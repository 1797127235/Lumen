...
class JDDiagnosis(Base):
    ...
    diagnosis_id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.user_id"), index=True)

    jd_text: Mapped[str] = mapped_column(Text)  # 参与诊断的原始 JD 文本；保留便于重跑诊断或审计
    jd_title: Mapped[str | None] = mapped_column(String(200))  # 展示用标题；LLM 或规则提取失败时可兜底为「未命名 JD」

    overall_score: Mapped[int] = mapped_column(
        Integer, default=0
    )  # 综合匹配分 0–100；看板卡片上的 match_score 通常与此一致

    summary: Mapped[str | None] = mapped_column(Text)  # 诊断摘要段落，供详情页与 LLM 建议 prompt 摘要使用
