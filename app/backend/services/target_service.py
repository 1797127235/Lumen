...
async def update_target(
    ...
    status_changed = (
        patch.status is not None and patch.status != target.status
    )
...
async def _load_diagnosis_dict(
    db: AsyncSession, diagnosis_id: str | None
) -> dict | None:
...
async def _aggregate_gaps(
    db: AsyncSession, diagnosis_ids: list[str]
) -> list[str]:
    ...
    result = await db.execute(
        select(JDDiagnosis).where(JDDiagnosis.diagnosis_id.in_(diagnosis_ids))
    )
